...
Workflow Overview
-----------------
This prompt guides you through a complete 7-steo QA workflow using MCP servers and AI agents to go from user story to commmitted automated test scripts.

Step1: Read User Story
-----------------------
Promopts:

I need to start a new testing workflow. Please read the use story from the file: user-stories/SCRUM-101-ecommerce-checkout.md
...
Step2: Create Test Plan
------------------------

Prompt:

Based on the user story SCRUM-101 that we just reviewwed, use the playwright-test-planner agent to:
...
3. Create a comprehensive test plan that covers all acceptance criteria including:
    ...
    - Negative scenarios(validation errors, emply fileds, invalid data)
    - Edge cases and boundary condidtions
...
Step3: Perform Exploratory Testing
----------------------------------
Prompt:
...
Then eecute the test scenarios defined in the plan:
1. Use Playwright browser tolls to manually execute each test scenario from the plan
...
Step4: Generate Automation Scripts:
------------------------------------
...
Please review:
1. Test plan from spects/saucedemo-checkout-test-plan,md(for test scenarios and steps)
2. Exploratory testing results from step3(for actual element selectors and UI insights)

Using insights from the manula exploratory testing:
- Leverage the element selectors and locators that were successfully used in step3
- Use stable element properties(IDs, data attributes, roles) discovred during exploration
- Apply wait strategies and UI behaviors observed during manual testing
- Incorporate any workarouds for UI quirks discovered

Generate Playwright Javascript automation scripts:
1. Create scripts for each test scenario from the test plan
2. Organize scriopts into appropriate test shite files in: tests/saucedemo-checkout/
3. Use the test case names and steps from the test plan
4. Use reliable selectors and strategies from exploratory testing

Requirements fro all scripts:
- Follow Playwright best practices
- Include proper assertions using epect()
...
- Add commments for complex steps
...
Step5: Execute and Heal Automation Tests
----------------------------------------
Prompt:
...
5. Repeat the heal proces until all tests are stable and passing
6. Document:
 - Initial test results(pass/fail count)
 - Healing activities performed
 - Final test resutls after healing
 - Any tests that couldn;t be autom-healed
...
Step6: Create Test Report
--------------------------
Prompt:

    Now I need to create a comprehensive test execution report based on manual testing, automation exection, and healing activities.
    ...
    3. Automated Test Results
        ...
        - Healing activities peroformed
        - Final test execution results after healing
        - Test suite execution summary
        - Pass/Fail count for each test suits

    4. Defects Log
        - For any failed tests(manual or automated):
            * Bug ID
            * Severity (Critical/High/Meium/Low)
            * Title and Description
            * Steps to Reproduce
            * Expected vs Actual behavior
            * Screenshots/Evidance
            * Environment Details
    5. Test Coverage Analysis
        - Which acceptance critoera are covered
...
Expected Output:
    ...
    Detailed bug report for failures
    Complete test coverage analysis
    Evidence and screenshots attached

Step 7: Commit to Git Repository
---------------------------------
Gut Repository URL: https://github.com/Veera26-git/AgentE2EQAWorkflow-Playwright.git

    Now I need to commit all the test artifacts to the Git repository using the GitHub MCP Server.

    Git Repository URL: https://github.com/Veera26-git/AgentE2EQAWorkflow-Playwright.git

    Please perform the following Git Operations:

    1. Initialize Git repository if not already initiated
    2. State all files in the workspace (all new and modified files)
    3. Create a commit with the message:
        "feat(tests): Add complete test suite for SCRUM-101 checkout workflow

        - Add user story documentaiton
...
Ecomplete Workflow Execution
----------------------------

Single Combiled Prompt(for video Demo):

    I Want to demonstrate a complete end-to-end QA workflow using natural language and MCP servers.

    STEP 1 - READ USER STORY:
    First, read the user story from: user-story/SCRUM-101-ecommerce-checkout.md
    Provide a brief summary of what needs to be tested.

    STEP 2 - CREATE TEST PLAN:
    Use the playwright-test-planner agent to create a comprehensive test plan based on the user story. The agent shoud explore the application URL from the user story and cover all acceptance criteria. Save it as: specs/saucedemo-checkout-test-plan.md
    ...
    Review both the test plan(specs/saucedemo-checkout-test-plan.md) and exploratory testing results from Step 3. Use the playwright-test-generator agent to crate JavaScript automation scripts leverating the element selectors and insights discovered during manual testing. Save scripts in tests/saucedemo-checkout/.
    ...
    Use the GitHub MCP agent to commit all new fields with a descriptive message and push to the repository.
